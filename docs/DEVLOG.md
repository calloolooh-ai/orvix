...
## calibration HUD had the same stale-screen-cache bug as the radial wheel

third time finding this exact shape now. `overlay.py`'s `CalibrationOverlayController._ensure_window` read `AppKit.NSScreen.mainScreen().frame()` once, at window creation, to park the calibration coverage HUD near the top of the main screen -- then never read it again. a calibration sweep runs SWEEP_SECONDS plus NEUTRAL_TILT_SECONDS, call it 19 seconds, which is real time for someone to unplug an external display or close/open a laptop lid mid-sweep. if the main screen changed size or position during that window, the HUD stayed parked at wherever the old screen used to be instead of following the new one. moved the origin calculation into `_show()` so it reads the screen fresh every time the HUD redraws, same fix already applied to the radial wheel's screen-height cache and main.py's desktop bounds. added a test documenting the stale-cache field is actually gone this time.

## restart-on-setting-change could silently no-op and get stuck on "restarting..."

every setter that only takes effect on pipeline start (cursor mode, multi monitor, extra gestures, radial dwell, profile load) shares one tail: `_restart_pipeline_if_running` sets the status to "restarting...", calls `worker.stop(wait=True)`, then `worker.start(...)`. `stop(wait=True)` joins the old background thread with a 2 second timeout so a wedged thread can't freeze the menu bar -- but it was ignoring whether that join actually succeeded. if the old thread didn't unwind in time, `start()` right after is a silent no-op by design (`PipelineWorker.start` refuses to run a second pipeline on top of one that's still alive), so the menu would sit on "restarting..." forever, the setting you just changed would never actually apply, and the stale pipeline would just keep running on the old settings with nothing telling you any of that. made `stop()` return whether the worker is actually confirmed stopped, and made every restart path check it: if the join times out, show a real error instead of quietly doing nothing.

## fourth spot with the same stale-screen-cache bug, this time in the two visualizers

grepped the whole repo for `mainScreen` after fixing the calibration HUD to make sure there wasn't a fourth instance hiding somewhere, and there was: `handviz.py`'s `FieldController` and `handrender.py`'s `HandController` both read `AppKit.NSScreen.mainScreen().frame()` exactly once in `_ensure_window`, then run their full-screen window at that size for as long as you leave the visualizer open. same story as the other three -- swap monitors or resize while `orvix viz` or `orvix hand` is running and the window stays pinned to the old screen's geometry, and every hand/ripple coordinate gets projected against the wrong size from then on. added a `_sync_screen_frame` check to both controllers' per-tick loop that compares the live screen frame against the window's current one and resizes both the window and its content view when they've drifted apart. this is the AppKit drawing path that's deliberately left untested (same as the rest of these two files, per the note at the top of their test files), so no new tests, just the fix.

## config writes weren't atomic

`save_config` is called constantly, basically every setter in `gui.py` calls it right after mutating a setting, plus profile save and every calibration write. it was writing straight into the real path with `yaml.safe_dump(data, f, ...)`. that's fine as long as the write always finishes, but a crash, force quit, or power loss landing mid-dump leaves a truncated or invalid yaml file sitting where the real config used to be. `load_config`'s callers already catch a broken config file and fall back to defaults so orvix doesn't refuse to start, but that fallback means losing every saved setting over one badly timed interruption, not just the specific change that was mid-write. switched `save_config` to write to a temp file in the same directory first and `os.replace()` it over the real path -- that's atomic on the same filesystem, so a reader only ever sees the fully-old or fully-new file, never something half written. added a test that checks the write round-trips correctly and doesn't leave a stray temp file behind.

## sanitizers crashing on the exact kind of bad value they exist to catch

stopped re-reading files that had already been read a few times over and actually ran malformed config values through `load_config` instead. every one of the "must be one of these strings" sanitizers -- `cursor_mode`, `preferred_hand`, `pinch_action`/`grab_action`, `thumbs_up_action`, and `radial_actions` -- does its check with `value in some_frozenset`. that works fine for a wrong *string*, which is the case they were all written for, but `in` on a set has to hash its argument first, and a hand-edited yaml value doesn't have to be a string at all. `cursor_mode: [absolute]` or `preferred_hand: {left: true}` parses fine as a list or dict, and `in` on those blows up with `TypeError: unhashable type` before the sanitizer ever gets to the fallback logic that's supposed to catch exactly this kind of nonsense. same deal for `radial_actions` being a non-list entirely (`radial_actions: 5` or `null`), which crashed even earlier since the code just tries to iterate it. all six spots now check the type first (`isinstance(value, str)` for the scalar fields, `isinstance(actions, list)` for radial_actions, plus a per-entry `isinstance` check before the membership test) and fall back the same way they already do for a valid-type-wrong-value input. added tests for each.

## the two order sanitizers had the same crash shape, just with a comparison instead of a set

kept fuzzing malformed config values since that just found six real crashes, and found two more spots with the exact same shape, just with `<=`/`>` instead of `in`. `_sanitize_calibration_axis_order` (used for all three of x/y/z min/max) and `_sanitize_tilt_deadzone_order` both assume their fields are already numbers and go straight to a comparison -- but unlike most of Settings, none of these six fields (the calibration box lives in its own nested dataclass, and tilt_full/tilt_deadzone aren't in any of the clamp tuples either) ever pass through `_clamp_field` first, which is the thing that normally catches a non-numeric value before anything downstream has to worry about it. `calibration: {x_min: "high"}` or `tilt_full: "nope"` hit the raw comparison as a string and crashed load_config outright, same "crashes on exactly the bad value it exists to catch" shape as the last batch. added an isinstance check to both before the comparison, falling back to the dataclass default for whichever side is bad. this is probably the last of this particular shape, since every field that reaches a numeric comparison in `_sanitize_settings` has now been checked one way or another, either by `_clamp_field` upstream or directly here.

## _drop_unknown_keys assumed the thing it was given was actually a mapping

went one level up from the individual field sanitizers and fuzzed the *shape* of the config file itself, not just individual values. `_drop_unknown_keys` does `set(raw) - known` then `raw.items()` to filter out typo'd keys -- both of those assume `raw` is a dict. a hand-edited `calibration: [1, 2, 3]`, `calibration: nope`, or `calibration: 5` gets popped out of the top-level yaml just fine (it parses as a list/string/int, no problem there), but then crashes with `AttributeError: 'list' object has no attribute 'items'` the moment `_drop_unknown_keys` tries to filter it, before `CalibrationBox(**calibration_raw)` ever runs. same story one level higher: if the *entire* config.yaml got replaced by a bare list or string (a botched hand edit, or copy-pasting the wrong file over it), `raw.pop("calibration", {})` in `load_config` crashes outright since `raw` itself isn't a dict. both spots now check `isinstance(raw, dict)` first and fall back to `{}` (or the full `Settings()` defaults for the top-level case) with a warning instead of taking the whole app down. added tests for both.

## profile names could pass validation and still crash the filesystem

fuzzed `load_config` a bunch more first to make sure the last few fixes actually closed the gap -- threw None/dict/string/list at every remaining sanitizer field and every one of them fell back cleanly this time, no crashes left in that path. moved on to `save_profile`/`load_profile` since that's the one part of the config layer that hadn't been fuzzed yet, and `_validate_profile_name` only ever checked the charset (letters, digits, `-`, `_`) and never the length. paste a couple hundred characters into the "save profile" text box in the menu bar app (or just hold a key down by accident) and that whole string is still valid charset-wise, so it sails past validation, and `gui.py`'s `_save_profile_as` only catches `ValueError` from that check -- anything else crashes straight through. `tempfile.mkstemp` inside `save_config` hits the filesystem's actual filename length limit and raises a raw `OSError` instead, which isn't caught anywhere, so the menu bar app would just die. capped profile names at 100 characters in `_validate_profile_name`, well under any real filesystem limit, so an overlong name now fails with the same clean "invalid profile name" message as any other bad name instead of taking the whole app down.

## a corrupted leapd frame could freeze the cursor forever, not just crash

went looking for the same "unvalidated input reaches something that can't recover from it" shape one layer further out, past config and profiles, at the actual sensor data. `leap_client.py` parses every websocket message with plain `json.loads`, and `json.loads` accepts the non-standard `NaN`/`Infinity`/`-Infinity` literals by default -- they're not valid JSON, but python's parser lets them through as constants. if leapd ever glitches and emits one of those for a coordinate (real hardware, real websocket, corruption happens), that frame parses just fine, `palmPosition` ends up with a `nan` in it, and it flows straight into the One Euro Filter with nothing checking it on the way in. tried it directly: feed the filter one NaN sample and every single output after that is NaN too, forever -- once `x_prev` goes NaN, `a * x + (1 - a) * x_prev` stays NaN no matter what real values come in afterward. the cursor would just freeze in place until you quit and restarted the whole app, no crash, no error, nothing to point at. added a `parse_constant` hook to the `json.loads` call in `stream_frames` that raises instead of accepting one of those literals, so a corrupted frame gets skipped with the same "got a non-json message from leapd" warning a malformed message already gets, rather than poisoning every filtered coordinate for the rest of the session.

## the zoom detector could hang on a huge-but-finite span instead of a NaN

kept pulling the "corrupted sensor data that's technically valid" thread. the NaN/Infinity fix closes off one way a bad frame can slip past json parsing, but a garbage-but-finite hand position (real number, wrong by many orders of magnitude, still hardware corruption territory) sails right through that guard since it's a perfectly normal float. traced where that could actually bite and found `_ZoomDetector.feed` in `extra_gestures.py`: it accumulates `span - self._last` into `self._resid` uncapped, then drains it with `while self._resid >= self._step: ...`. `zoom_step_mm` is only floored at 0.1mm (see the comment on `_POSITIVE_STEP_FIELDS`), so a span that jumps by, say, 1e18mm in one frame turns that drain loop into roughly 1e19 iterations. tried it directly and it hung immediately, same dispatch-thread freeze as the old zoom-step-of-zero bug, just reached from the input side instead of the config side. no real hand closes or opens by more than a few mm between frames, so clamped the delta to ±200mm before it ever reaches `resid` -- a garbage frame still produces a step or two of zoom, which is harmless, instead of a hang. `_VolumeTwistDetector` didn't need the same fix since its delta already gets wrapped into `(-pi, pi]` to handle the atan2 seam, which bounds its drain loop for free.

double checked the sibling spots afterward: coord_mapper.py already clamps x/y to screen bounds before the int cast, so a garbage-huge position can't overflow the mouse-move call either way; the One Euro Filter produces a huge-but-finite (not inf/nan) output for a huge-but-finite input, which still gets clamped downstream; and the volume twist detector, atan2/hypot calls, and the websocket layer itself all checked out fine. this thread looks mined out too.

## a stuck radial wheel or dwell ring could stay frozen on screen after Stop

`run_live`'s frame loop only ever hides the radial wheel overlay or the cursor dwell ring by calling `on_radial(None)`/`on_dwell(None)` on its own closing frame -- when the wheel closes normally, or when the dwell countdown finishes or gets interrupted. but the loop has no `finally` around it, only a bare `except LeapConnectionError`. cancelling the pipeline (clicking Stop, or any setting change that goes through `_restart_pipeline_if_running`) raises `asyncio.CancelledError` instead, which skips straight past all of that and out of the function. if the cancel landed while the radial wheel happened to be open, or the dwell ring was mid countdown, the click-through overlay window it was drawing into just stayed there, frozen on whatever it last rendered, until something else happened to draw over it or the app quit. a real, reachable visual/logical desync, same family as the mutual-exclusion races fixed earlier but on the way out instead of the way in. added a `_hide_live_overlays` helper in `gui.py` that renders both overlays to `None` and calls it from `_toggle_running`'s stop branch and from `_restart_pipeline_if_running`, so stopping the pipeline always leaves the screen clean no matter what the wheel or ring were doing at the moment it got cut off. added tests for both paths.

## "skip for now" on the welcome screen didn't actually skip anything long term

first-run onboarding shows a welcome alert with two choices: calibrate now, or skip for now. picking calibrate saves your settings at the end of the sweep same as any other calibration, so `is_first_run` sees a real config.yaml on the next launch and stays quiet. picking skip did nothing at all -- no config write, nothing saved, the callback just returned. `is_first_run` only looks for a config.yaml or a saved profile, neither of which existed, so if you skip and don't happen to touch some other menu setting before quitting, the exact same welcome alert pops right back up on the next launch. and the one after that. forever, until you either calibrate or stumble into a setting that happens to call `save_config`. `onboarding.py`'s own docstring says skipping the wizard should make this go false for good, so this was actually a regression against the documented intent, not just an oversight. fixed by saving the current settings when the user picks skip, same as any other setting change already does. no test at all existed for `_maybe_show_onboarding` before this, so added three: skip saves, calibrate doesn't double-save on top of the calibration flow's own save, and a later run with `_is_first_run` already false doesn't even show the alert.

## cancelling the pipeline mid-drag could leave the real mouse button stuck down

kept pulling the "abnormal exit path skips cleanup" thread from the last fix, since it just found a real bug. that one was about an overlay window; this time it's worse. `run_live`'s frame loop only calls `interpreter.reset()` (which emits release events for anything mid-hold) on the radial-close and hand-loss paths -- it has no `finally`, only `except LeapConnectionError`. clicking Stop, or any setting change that goes through `_restart_pipeline_if_running`, cancels the task instead, and `asyncio.CancelledError` can land on any await point in the loop, including right after a real `mouse_down()` went out for a pinch-drag that's still mid-hold. none of the existing cleanup runs, so the cancellation just propagates and the physical mouse button stays held down until some unrelated later click happens to release it. added an `except asyncio.CancelledError` around the frame loop that calls `interpreter.reset()` and dispatches whatever release events come back before re-raising, same machinery the radial-close path already leans on, just wired to the one exit path that never got it. added a test that fakes a mid-drag cancel and checks `mouse_up` actually fires.

after that, spent a whole cycle re-verifying the three worst fixes with actual repro scripts instead of just rereading the diffs -- the cancellation cleanup re-raises properly instead of swallowing it, the NaN rejection only skips the one bad frame and leaves the connection alive, and the zoom clamp returns in well under a second on a 1e18 span. all held up, no gaps.

## leapd connecting-but-never-answering could hang the whole app forever

`bin/orvix`'s own `status` subcommand wraps its leapd probe in `asyncio.wait_for(..., timeout=3)`, but the actual connection code every other part of the app uses -- `leap_client.py`'s `stream_frames`, which `main.py`, `gui.py`, `calibration.py`, `handviz.py`, and `handrender.py` all go through -- just did `await websockets.connect(url)` with nothing bounding it. leapd not running at all fails that almost instantly with a normal `OSError`, already handled. but a leapd process that's up and listening on the port while wedged somewhere in its own handshake logic (or literally anything else squatting on 6437 that accepts the TCP connection but never speaks websocket) would hang that await forever. no error, no timeout, the menu bar app just sits on "starting..." with nothing to click and no way to know why short of force quitting and checking Activity Monitor. added a 5 second timeout around the connect call, same idea as the status check already had, raising the existing `LeapConnectionError` with a message pointing at the `launchctl kickstart` fix instead of hanging. added a test that fakes a connect that never returns and confirms it times out instead of hanging the test suite.

## and the exact same hang, one step further downstream: leapd going quiet mid-session

fixing the connect hang made the obvious next question: what if leapd finishes the handshake fine and then goes quiet later, not at startup? leapd streams frames at 75+ fps continuously once connected, even an empty-hands frame arrives every tick, so silence for more than a moment is already abnormal. the frame-reading loop was a plain `async for raw_message in ws`, which has no timeout of its own -- if leapd's process is still alive and still answering the websocket library's own ping/pong keepalive (so it never looks disconnected at the transport level) but has simply stopped producing frames, that loop just sits there waiting for the next message forever. exact same "hang with zero feedback" shape as the connect bug, just further downstream and harder to notice since the app looks like it's running right up until you realize the cursor stopped responding minutes ago. swapped the bare iteration for a manual loop that wraps each `ws.recv()` in `asyncio.wait_for` with a 5 second idle timeout, raising the same `LeapConnectionError` (with its own message pointing at the same kickstart fix) instead of hanging. a clean disconnect still returns normally same as before, only true silence times out. added a test with a fake websocket whose `recv()` never returns, confirming it raises instead of hanging the suite.
